...
const ICO_MIME_TYPES = ["image/x-icon", "image/vnd.microsoft.icon"];
...
const translations = {
  en: {
    ...
    subtitle:
      "Convert your ICO icon files to JPEG or PNG images entirely in your browser. No server uploads — all processing is local and private.",
    tabConvert: "Convert",
    tabHistory: "History",
    dropTitle: "Drag & drop ICO files here",
    dropOr: "or",
    ...
    featBatchDesc: "Upload multiple ICO files and convert them all at once.",
    footer: "Insait · ICO to Image Converter",
    ...
    hintLarge: "Shrink the output to 75% while keeping the aspect ratio.",
    hintMedium: "Shrink the output to half of the original dimensions.",
    hintSmall: "Create a compact image at 25% of the original size.",
    hintCustom: "Specify exact pixel dimensions for the output image.",
    ...
    loadImageFailed: "Failed to load ICO file.",
    outputSizeLabel: "Output size: {label}.",
  },
  de: {
    title: "Insait ICO-zu-Bild-Konverter",
    subtitle:
      "Konvertieren Sie Ihre ICO-Icon-Dateien vollständig im Browser in JPEG- oder PNG-Bilder. Kein Server-Upload — die gesamte Verarbeitung erfolgt lokal und privat.",
    tabConvert: "Konvertieren",
    tabHistory: "Verlauf",
    dropTitle: "ICO-Dateien hier ablegen",
    dropOr: "oder",
    ...
    featBatchDesc:
      "Laden Sie mehrere ICO-Dateien hoch und konvertieren Sie alle auf einmal.",
    footer: "Insait · ICO-zu-Bild-Konverter",
    sizeOriginal: "Original (100 %)",
    sizeLarge: "Groß (75 %)",
    ...
    hintLarge: "Ausgabe auf 75 % verkleinern, Seitenverhältnis bleibt erhalten.",
    hintMedium: "Ausgabe auf die halben Originalabmessungen verkleinern.",
    hintSmall: "Kompaktes Bild mit 25 % der Originalgröße erstellen.",
    hintCustom: "Genaue Pixelabmessungen für das Ausgabebild festlegen.",
    ...
    loadImageFailed: "ICO-Datei konnte nicht geladen werden.",
    outputSizeLabel: "Ausgabegröße: {label}.",
  },
} as const;
...
/** Convert an ICO File to JPEG or PNG Blob entirely in the browser using the Canvas API. */
function convertIcoToImage(
  file: File,
  format: OutputFormat,
  ...
  return new Promise((resolve, reject) => {
    ...
    img.onload = () => {
      try {
        ...
        // Fill white background for JPEG (which has no transparency)
        if (format === "jpeg") {
...
export default function HomePage() {
  ...
  const addFiles = useCallback((incoming: FileList | File[]) => {
    ...
    const ico = arr.filter((f) =>
      ICO_MIME_TYPES.includes(f.type) || f.name.toLowerCase().endsWith(".ico")
    );
  ...
  const convertAll = async () => {
    ...
    for (const item of pending) {
      ...
      try {
        const blob = await convertIcoToImage(item.file, outputFormat, dimensions, t);
            ...
            >
              <div className="flex flex-col items-center gap-4">
                ...
                <input
                  ref={inputRef}
                  type="file"
                  accept={[...ICO_MIME_TYPES, ".ico"].join(",")}
                  multiple
                  className="hidden"
                  ...
                  <div className="flex flex-col gap-1 sm:flex-row sm:items-start sm:justify-between">
                    <div>
                      ...
                      <p className="text-gray-500 text-xs mt-1">
                        {t.outputSizeLabel.replace("{label}", currentSizeLabel)}{" "}
                        {currentSizeHint}
                      </p>
                    </div>
